    ...
    static getUserByID(userID) {
      ...
      getStaffByID(staffID) {
        try {
          ...
          const staff = User.getUserByID(staffID); 
          if (!staff) {
      ...
      updateUser(propertyName, newValue) {
        try {
          if (!this.getIsActive()) {
            throw new Error("Inactive users cannot update properties.");
          }
      
         
          if (!this.isAdmin) { 
            throw new Error("Only admins can update user properties.");
          }
      
          switch (propertyName) {
            case "firstName":
              this.updateFirstName(newValue);
              break;
            case "lastName":
              this.updateLastName(newValue);
              break;
            default:
    ...
    getContactByID(contactID) {
        try {
          ...
          for (let i = 0; i < this.contacts.length; i++) {
            ...
            if (contact.getContactID() === contactID && contact.isActive) {
              return contact;
            }
          }
